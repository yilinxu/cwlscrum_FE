...
def run_command(cmd, logger=None, shell_var=False):
    '''
    Runs a subprocess
    and log the subprocess command, stdout and stderr
    return the exit_code for the command
    '''
...
def get_interval_region(reference_intervals, workdir, chunk, num_intervals):
    ...
    if '.intervals' in reference_intervals:
        chunked_intervals = os.path.join(workdir, os.path.basename(reference_intervals).replace('.intervals', '.%d.intervals' % (chunk)))
    elif '.bed' in reference_intervals:
        chunked_intervals = os.path.join(workdir, os.path.basename(reference_intervals).replace('.bed', '.%d.bed' % (chunk)))

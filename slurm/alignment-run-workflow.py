...
def run_cwl(args, statusclass, metricsclass):
    ...
    jobdir = tempfile.mkdtemp(prefix="bam_%s" % output_uuid, dir=args.base_dir)
    ...
    for ref in refs:
        local_ref = os.path.join(refdir, os.path.basename(ref['s3_url']))
        print(local_ref)
        print(ref['s3_url'])
        print(ref['s3_profile'])
        print(ref['s3_endpoint'])
